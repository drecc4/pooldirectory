...
def render_transposed_neighborhood_breakdown(scope: pd.DataFrame):
    """Same data as render_neighborhood_breakdown but transposed:
    metrics as rows, neighborhoods as columns. Designed to fit in a narrow
    side column next to the map."""
    ...
    for nbhd in nbhds:
        ...
        rows.append({
            ...
            "Total": f"{total:,}",
            "Marked": f"{marked:,}",
            "Marked %": f"{round(marked / total * 100)}%" if total else "—",
            "Pools": f"{yes:,}",
            "Pool %": f"{yes / total * 100:.1f}%" if total else "—",
        })

    df = pd.DataFrame(rows).set_index("_nbhd")
    transposed = df.T.reset_index().rename(columns={"index": "Metric"})
